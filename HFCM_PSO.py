...
def generate_seq(true_seq, weight, Nc, K, Order):
    ...
    for sample in range(Order, K-1):  # iterate each sample
        for j in range(Nc):    # iterate each node j
            ...
            beta = weight[j * len_var + Order * Nc + 1]  # beta
...
def objfunc_PSO(x, *args):
    Nc, K, Order, belta, true_seq = args
    seq = generate_seq(true_seq, x, Nc, K, Order)
...
def HFCM_PSO():
    ...
    true_seq = U_train
    args = (Nc, len_train_data, Order, belta, true_seq)
    # lb = [lowBund] * Nc
    # ub = [uppBnd] * Nc
    ...
    trainPredict = generate_seq(true_seq, xopt, Nc, len_train_data, Order)
    fig1 = plt.figure()
    ax1 = fig1.add_subplot(211)
    # fig1.hold()
    for i in range(Nc):
